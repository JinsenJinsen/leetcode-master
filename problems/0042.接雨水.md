...
# 动态规划
        ...
        for i in range(1, len_-1):
            sum_1 = min(lheight[i], rheight[i]) - height[i]
            if sum_1 > 0:
                sum_ += sum_1
...
class Solution {
    public int trap(int[] height) {
        ...
        for (int i = 0; i < height.length; i++) {
            ...
            int sum1 = Math.min(lheight, rheight) - height[i];
            if (sum1 > 0) {
                sum += sum1;
            }
        }
...
//动态规划
        ...
        for (int i = 1; i < len-1; i++) {
            int sum1 = Math.min(lheight[i], rheight[i]) - height[i];
            if (sum1 > 0) {
                sum += sum1;
            }
        }
